...
class Fuelcell:
    ...
    def tempResponse(self, I, SH2, SO2, T, P, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False, overpotential = 'E'):
        polCurves = []
        P = 1.59 #Base case pressure
        IEC_mem = 50
        IEC_io = 1.72
        delta_mem   = 0.005
        CO_H2       = 0
        ...
        for p in range(len(T)):
            ...
            polCurve_q = np.asarray(self.operate(I, SH2, SO2, T_i, P, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False))
        ...
        overpotentialDict = {'Eoc': 0, 'E': 1, 'Eact': 2, 'Econ': 3, 'Eohm': 4, 'Contributions': 5, 'Power' : 6}
        overpotential = overpotentialDict[overpotential]
        reverseOverpotentialDict = {v: k for k, v in overpotentialDict.items()}
        
        plt.figure(figsize = (20, 15))
        
        if overpotential == 6:
            for p in range(len(T)):
                plt.plot(I/self.A, polCurves[p, 1, :]*I/self.A, linestyle = '-', linewidth = 2)
        
            labels = T
            labels = [str(item) for item in labels]
            unit = ' °C'
            labels = [x + unit for x in labels]
            
            plt.ylim(0, 1.2)
            plt.xlim(0, 2.5)
            plt.legend(labels, fontsize = 'xx-small', loc='upper right', ncol = 3)
            tnfont={'fontname':'Times New Roman'}
            plt.rcParams['font.family']='Times New Roman'
            plt.rcParams['font.size']=35
            plt.xlabel('Current intensity (A/cm2)')
        ...
        if overpotential == 5:
            labels = []
            linestyle = ['-', '-', '-.', ':', '-']
            temp = [str(t) for t in T]  
            unit = ' °C'
            temp = [t + unit for t in temp]
            
            for ip in range(1, overpotential):
                for p in range(len(T)):
                    plt.plot(I/self.A, polCurves[p, ip, :], linestyle = linestyle[ip], linewidth = 3)
                    labels.append(reverseOverpotentialDict[ip] + temp[p])
                
            labels = [item for item in labels]
            
            labels.append('Experimental (220 °C)')          
            plt.scatter(J1, E_exp1, label = 'Experimental (220 °C)', marker = 'x')
            
            labels.append('Experimental (200 °C)')          
            plt.scatter(J2, E_exp2, label = 'Experimental (200 °C)', marker = '^')
        ...
        if overpotential == 1:
            for p in range(len(T)):
                plt.plot(I/self.A, polCurves[p, overpotential, :])
            ...
            labels.append('Base case 2 (200 °C)')          
            plt.scatter(J2, E_exp2, label = 'Experimental (200 °C)', marker = '^')
        ...
        if overpotential ==  0 or overpotential ==  2 or overpotential ==  3 or overpotential ==  4:
    ...
    def pressureResponse(self, I, SH2, SO2, T, P, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False, overpotential = 'E'):
        polCurves = []
        T = 493 #Base case temperature
        IEC_mem = 50
        IEC_io = 1.72
        delta_mem   = 0.005
        CO_H2       = 0
        ...
        for p in range(len(P)):
            ...
            polCurve_q = np.asarray(self.operate(I, SH2, SO2, T, P_i, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False))
            
            polCurves.append(polCurve_q)
            
        polCurves = np.asarray(polCurves)
        
        #Load data for base case (220 °C, 1.59 bar)
        data1 = data_load('/Validation data - LBM.xlsx')
        J1 = np.asarray(pd.DataFrame(data1, columns=['I (A/cm2) 220']))
        E_exp1 = np.asarray(pd.DataFrame(data1, columns=['E (V) 220']))
        ...
        if overpotential == 5:
            ...
            labels.append('Experimental (1.6 atm)')          
            plt.scatter(J1, E_exp1, label = 'Experimental (1.6 atm)', marker = 'x')
        ...
        if overpotential == 1:
            ...
            labels.append('Experimental (1 bar)')          
            plt.scatter(J1, E_exp1, label = 'Experimental (1 bar)', marker = 'x')
    ...
    #CO Response
    def coResponse(self, I, SH2, SO2, T, P, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False, overpotential = 'E'):
        polCurves = []
        T = 493 #Base case temperature
        P = 1.59 #Base case pressure
        IEC_mem = 50
        ...
        for p in range(len(CO_H2)):
            CO_H2_i = CO_H2[p]
                  
            polCurve_q = np.asarray(self.operate(I, SH2, SO2, T, P, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2_i, L_c, params, params_names, graphs = False))
        ...
        #Load data for base case (220 °C, 1.59 bar)
        data = data_load('/Validation data - LBM.xlsx')
        J = np.asarray(pd.DataFrame(data, columns=['I_CO(A/cm2)220']))
        E_exp = np.asarray(pd.DataFrame(data, columns=['E_CO(V)220']))
        ...
        if overpotential == 5:
            ...
            labels.append('Experimental 0 C0/H2')          
            plt.scatter(J1, E_exp1, label = 'Experimental 0 C0/H2', marker = 'x')
            
            labels.append('Experimental 0.33 CO/H2')
            plt.scatter(J, E_exp, label = 'Experimental 0.33 CO/H2', marker = '^')
        ...
        if overpotential == 1:
            ...
            labels.append('Experimental 0.33 CO/H2')
            plt.scatter(J, E_exp, label = 'Experimental 0.33 CO/H2', marker = 'x')
    ...
    #delta_mem Response
    def delta_memResponse(self, I, SH2, SO2, T, P, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False, overpotential = 'E'):
        polCurves = []
        T           = 493   #K
        P           = 1.59  #atm
        CO_H2       = 0
        ...
        for ix in range(len(delta_mem)):
            delta_mem_i = delta_mem[ix]
                  
            polCurve_q = np.asarray(self.operate(I, SH2, SO2, T, P, IEC_mem, IEC_io, delta_mem_i, delta_io, CO_H2, L_c, params, params_names, graphs = False))
        ...
        if overpotential == 5:
            ...
            labels.append('Experimental 0.005 cm')          
            plt.scatter(J1, E_exp1, label = 'Experimental 0.005 cm', marker = 'x')
    ...
    #IEC_mem Response
    def IEC_memResponse(self, I, SH2, SO2, T, P, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False, overpotential = 'E'):
        polCurves = []
        T           = 493   #K
        P           = 1.59  #atm
        CO_H2       = 0
        ...
        for jx in range(len(IEC_mem)):
            IEC_mem_i = IEC_mem[jx]
                  
            polCurve_q = np.asarray(self.operate(I, SH2, SO2, T, P, IEC_mem_i, IEC_io, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False))
        ...
        if overpotential == 5:
            ...
            labels.append('Experimental 50 %QPPSf')          
            plt.scatter(J1, E_exp1, label = 'Experimental 50 %QPPSf', marker = 'x')
    ...
    #DF_io Response
    def IEC_ioResponse(self, I, SH2, SO2, T, P, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False, overpotential = 'E'):
        polCurves = []
        T           = 493   #K
        P           = 1.59  #atm
        CO_H2       = 0
        ...
        for w in range(len(IEC_io)):
            IEC_io_i = IEC_io[w]
                  
            polCurve_q = np.asarray(self.operate(I, SH2, SO2, T, P, IEC_mem, IEC_io_i, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False))
        ...
        if overpotential == 5:
            ...
            labels.append('Experimental 100 %QPPSf')          
            plt.scatter(J1, E_exp1, label = 'Experimental 100 %QPPSf', marker = 'x')
    ...
    #L_c Response
    def L_cResponse(self, I, SH2, SO2, T, P, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2, L_c, params, params_names, graphs = False, overpotential = 'E'):
        polCurves = []
        T           = 493   #K
        P           = 1.59  #atm
        CO_H2       = 0
        ...
        for w in range(len(L_c)):
            L_c_i = L_c[w]
                  
            polCurve_q = np.asarray(self.operate(I, SH2, SO2, T, P, IEC_mem, IEC_io, delta_mem, delta_io, CO_H2, L_c_i, params, params_names, graphs = False))
        ...
        if overpotential == 5:
            ...
            labels.append('Experimental 0.5 mgPt/cm2')          
            plt.scatter(J1, E_exp1, label = 'Experimental 0.5 mgPt/cm2', marker = 'x')
        ...
        if overpotential == 1:
            ...
            labels.append('Experimental 100 mgPt/cm2')          
            plt.scatter(J1, E_exp1, label = 'Experimental 100mgPt/cm2', marker = 'x')
